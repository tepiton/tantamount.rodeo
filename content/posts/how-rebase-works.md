...
excerpt:  Rebasing has always confused me.
...
Rebasing is one way of   ["integrating changes from one branch into another."](https://git-scm.com/book/en/v2/Git-Branching-Rebasing)
...
> Tack `<branch>` onto the end of `<upstream>`.

If you don't specify `<branch>`, rebase uses the current branch.
...
According to its [man page](https://git-scm.com/docs/git-rebase), rebase does its work in four steps:
...
3. Reset the _branch to rebase_ to the _branch to rebase onto.   That means setting `topic` (which is also the current branch) to the same commit as `master`.
